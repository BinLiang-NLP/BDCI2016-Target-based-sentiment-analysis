...
def Seg(paragraph):
    ...
    for a in atoms:
        ...
        try:
            yield (i.decode('utf-8'), a.sPOS.decode('utf-8'))
        except:
            yield ('', '')
...
def cut_sentence(sentence, string=False):
    ...
    if not string:
        words = []
        for word in Seg(sentence.encode('utf-8')):
            words.append(word)
        return words
    else:
        words = []
...
# 添加用户自定义字典
count = ImportUserDict(b'./external_data/sentiment_word/userdict1.csv', ENCODING.UTF8_CODE,b'')
print('成功加载自定义词典%d条！'%count)

def demo():
    s = "今天 天气 不错"

    # return: [('今天', 't'), ('天气', 'n'), ('不错', 'pos')]
    print(cut_sentence(s, string=False))
    
    # return: 今天/t 天气/n 不错/pos
    print(cut_sentence(s, string=True))
